...
local function isAdmin(source)
    ...
    elseif Config.Framework == 'qb-core' then
        ...
        if Player then
            for _, group in ipairs(Config.AdminGroups) do
                if Player.PlayerData.job.name == group or Player.PlayerData.job.grade.level >= 3 then
                    return true
                end
            end
        end
...
AddEventHandler('KF_Reports:BringPlayer', function(playerId)
    ...
    if isAdmin(src) then
        ...
        if targetPlayer then
            ...
            TriggerClientEvent('KF_Reports:Notify', playerId, 'An admin has brought you to them.', 'info')
        else
...
AddEventHandler('KF_Reports:TeleportPlayer', function(playerId)
    ...
    if isAdmin(src) then
        ...
        if targetPlayer then
            ...
            TriggerClientEvent('KF_Reports:Notify', playerId, 'An admin has teleported to you.', 'info')
        else
            TriggerClientEvent('KF_Reports:Notify', src, 'Player not found.', 'error')
        end
    else
